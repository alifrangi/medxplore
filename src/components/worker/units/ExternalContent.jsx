...
const ExternalContent = ({
  ...
  const permissions = UNIT_PERMISSIONS.external;
  ...
  const handleIdeaClick = (idea) => {
    ...
    const viewOnly = idea.currentUnit !== 'external';
  ...
  const handleApprove = async (ideaId, notes) => {
    try {
      ...
      toast.success('Idea approved and forwarded to Systems Unit');
      handleCloseModal();
    } catch (error) {
  ...
  const handleReturn = async (ideaId, reason) => {
    try {
      ...
      toast.success('Idea returned to Operations');
      handleCloseModal();
    } catch (error) {
      ...
      <div className="unit-content__header">
        <div className="unit-content__title">
          <span className="unit-content__icon">
            <Icon name={unit?.icon || 'Building2'} size={28} />
          </span>
          <div>
            <h1>{unit?.name || 'External Approvals'}</h1>
      ...
      {filteredIdeas.length > 0 ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state__description">
            {activeTab === 'pending'
              ? 'Ideas approved by Operations will appear here'
