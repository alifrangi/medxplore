...
import { PIPELINE_STAGES, IDEA_TYPES, UNIT_PERMISSIONS, getTimeInStatus, getNextStage } from '../../../data/mockData';
...
const AcademicContent = ({
  ...
  const permissions = UNIT_PERMISSIONS.academic;
  ...
  const handleIdeaClick = (idea) => {
    ...
    // View-only if the idea is not in the current unit
    const viewOnly = idea.currentUnit !== 'academic';
  ...
  const handleApprove = async (ideaId, notes, driveLink) => {
    try {
      // Save drive link first if provided
      if (driveLink) {
        await onUpdateDriveLink(ideaId, driveLink);
      }
      await onApprove(ideaId, notes);
      toast.success('Idea approved and forwarded to Programs Unit');
      handleCloseModal();
    } catch (error) {
  ...
  const handleSaveDriveLink = async (ideaId, driveLink) => {
    try {
      await onUpdateDriveLink(ideaId, driveLink);
      ...
      <div className="unit-content__header">
        <div className="unit-content__title">
          <span className="unit-content__icon">
            <Icon name={unit?.icon || 'BookOpen'} size={28} />
          </span>
          <div>
            <h1>{unit?.name || 'Academic Unit'}</h1>
      ...
      {filteredIdeas.length > 0 ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state__description">
            {activeTab === 'pending'
              ? 'New ideas will appear here when they are submitted'
